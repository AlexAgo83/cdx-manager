...
- Codex does not read hooks from a file at all. A hooks file at the Codex home root, under a `hooks/` subdirectory, or in a project `.codex/` is never loaded — confirmed by testing all three, and corroborated by every trust key in `config.toml` being of the form `plugin@marketplace:path:event`, a shape a free-standing file cannot have. Codex runs only hooks that come from an **installed plugin**, so cdx has to generate one and install it with `codex plugin marketplace add`, which accepts a local path.
- The generated plugin must be rooted **outside** `CODEX_HOME`. Rooted inside it, the marketplace installs with no error and reports `installed, enabled`, and then never runs: Codex registers it pointing at the source directory rather than the cache copy it executes from. This is the sharpest silent-success trap in the whole request — everything looks correct and nothing happens.
...
- Verified against a real turn: Codex sends the same payload shape as Claude Code, on standard input, carrying `hook_event_name`, `cwd` and `last_assistant_message`. One reader serves both. The older `notify` key, which Codex now calls `legacy_notify`, passed its payload as an argument instead; accepting that shape too costs a line and covers anyone still on it.
...
- AC17: A host where the provider's own CLI cannot run leaves provisioning skipped and the launch unaffected, rather than failing it.
- AC18: The README documents the notification behavior, how to turn it off, the new meaning of `cdx notify`, and the per-platform delivery caveats including WSL.
