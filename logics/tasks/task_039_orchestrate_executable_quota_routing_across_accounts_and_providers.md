...
- request-AC11 -> This task. Proof: **observed, after two corrections.** The first live trial failed and the failure was worth more than the feature: the provider ignores `--session-id` under `--bg` and assigns its own (cdx asked for `6e8e9e0b-...`, got `373c3949-...`), so cdx could not identify the agent it had just started, fell back to the in-house launcher, and ran the same task twice - once untracked. The reported `pid` also belonged to a `claude bg-spare` daemon helper and went `null` within a minute. Both are fixed: the agent is named from the `backgrounded - <id>` line `--bg` prints, and a launch that reports no id now fails loudly instead of falling back, since the first agent may still be running. A second correction followed: completion is not readable from agent state at all. A `--bg` session answers and then parks at `state: blocked, status: idle` indefinitely, waiting for a next prompt a one-shot run never sends - observed on a run whose transcript held `result: OK` and 32 327 tokens while the agent reported blocked for over two minutes. Completion is therefore read from the provider's own session transcript (`.claude/projects/<cwd>/<session-id>.jsonl`, carrying the answer and `message.usage`), and the parked session is stopped once its task is done. Verified end to end on a live account: launch, automatic closeout with `result: OK` and 32 796 tokens, and no session left running. The criterion is met. Delegation remains behind `CDX_EXPERIMENTAL_NATIVE_BG=1` because it has been exercised on one account with one prompt shape, not because it is known broken - an exposure judgement, not a correctness one.
