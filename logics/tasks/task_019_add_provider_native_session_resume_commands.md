...
```mermaid
%% logics-kind: task
%% logics-signature: task|add-provider-native-session-resume-comma|item-020-add-provider-native-session-res|1-add-a-provider-neutral-resume-capabili|planned
stateDiagram-v2
...
# Plan
- [ ] 1. Add a provider-neutral resume capability helper that returns `resumable`, `provider`, `strategy`, `reason`, and a redacted provider command preview for a session.
- [ ] 2. Add provider runtime support for an interactive `resume` action distinct from normal `launch`.
- [ ] 3. Implement Codex resume spec as `codex resume --last` under the session's isolated `CODEX_HOME`, preserving cwd with `--cd` and compatible launch settings.
- [ ] 4. Implement Claude resume spec as `claude --continue` under the session's isolated `HOME`, preserving cwd through process options and compatible launch settings.
- [ ] 5. Add CLI parsing for `cdx <name> -r`, `cdx <name> --resume`, `cdx resume <name> [--json]`, and `cdx can-resume <name> [--json]`.
- [ ] 6. Ensure unsupported providers report `not_supported` from capability checks and fail resume launch without starting a normal launch.
- [ ] 7. Preserve existing auth checks, disabled-session checks, state rehydration, runtime lifecycle recording, launch history recording, and JSON success/error envelopes.
- [ ] 8. Update README command table and `_print_help` output with the new commands and flags.
- [ ] 9. Add focused tests for CLI parsing, provider specs, can-resume JSON, unsupported providers, and normal launch regressions.
- [ ] 10. Run validation and update linked Logics docs with proof before closeout.
...
- [ ] README and CLI help are aligned.
- [ ] Validation passes.
- [ ] Linked request/backlog/task docs are updated with final evidence.
- [ ] Status is `Done` and progress is `100%` only after implementation and validation.
...
- Planned:
  - `npm run lint`
  - `npm test`
  - Focused `python -m unittest discover -s test -p 'test_*_py.py' -k resume`
  - `logics-manager lint --require-status`
  - `logics-manager audit`

# Report
- Not implemented yet. This task is ready for development.
