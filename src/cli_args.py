...
AUTH_USAGE = "Usage: cdx auth refresh <name|all> [--json]"
...
HANDOFF_USAGE = "Usage: cdx handoff <name> [--json] | cdx handoff <source> <target> [--source-conversation ID | --source-transcript PATH] [--json]"
...
TRAY_USAGE = "Usage: cdx tray status [--json] [--refresh] [--beat] | cdx tray install [--json] [--yes] [--autostart|--no-autostart] [--alerts|--no-alerts] | cdx tray launch|uninstall|doctor [--json] | cdx tray autostart [on|off|status] [--json] | cdx tray events [--json] | cdx tray ack <id>... [--json] | cdx tray heartbeat [--json] | cdx tray alerts [on|off|status] [--json] | cdx tray plugin [status|enable <name>|disable <name>|run <action>] [--json] | cdx tray terminal [status|list|set <app>|clear] [--json] | cdx tray sort [status|set <capacity|reset|recent>] [--json]"
...
def _parse_set_args(args):
    parsed = _parse_flag_args(args, {
        ...
        "--notify-preview": {"key": "notify_preview", "type": "str", "default": None, "transform": _parse_fast_value},
        "--model": {"key": "model", "type": "str", "default": None},
    ...
    settings = {
        key: parsed[key]
        for key in ("power", "permission", "fast", "rtk", "logics", "notify", "notify_preview", "model", "fallback_model", "budget", "extra_args", "priority")
...
def _parse_unset_args(args):
    parsed = _parse_flag_args(args, {
        ...
        "--notify-preview": {"key": "notify_preview", "type": "bool", "default": False},
    ...
    if not parsed["names"] and not parsed["sessions"] and not parsed["provider"]:
        raise CdxError(UNSET_USAGE)
    _UNSET_KEYS = (
        "power", "reasoning_effort", "permission", "fast", "rtk", "logics", "notify", "notify_preview",
...
def _parse_auth_args(args):
    parsed = _parse_flag_args(args, {
        "--json": {"key": "json", "type": "bool", "default": False},
    }, AUTH_USAGE, positionals_key="values", max_positionals=2)
    if len(parsed["values"]) != 2 or parsed["values"][0] != "refresh":
        raise CdxError(AUTH_USAGE)
    target = parsed["values"][1]
    if not target.strip():
        raise CdxError(AUTH_USAGE)
    return {"target": target, "json": parsed["json"]}
