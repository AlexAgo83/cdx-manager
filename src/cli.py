...
def _print_help(use_color=False):
    return "\n".join([
        ...
        f"  {_style('cdx status [--json] [--refresh|--cached]', '36', use_color)}",
        f"  {_style('cdx status --small|-s [--refresh|--cached]', '36', use_color)}",
        f"  {_style('cdx status <name> [--json] [--refresh|--cached]', '36', use_color)}",
