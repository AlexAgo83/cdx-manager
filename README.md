...
- **Native background where the provider has it.** `cdx run --detach` uses Claude Code's own background agents (`--bg`, `claude agents`) when the installed CLI supports both starting and reporting them, and falls back to cdx's own detached launcher otherwise. Decided by asking the CLI what it accepts, never by comparing versions. The run keeps its cdx `run_id` either way, and the record names which path carried it. Codex keeps the in-house path while its `cloud`/`exec-server` surfaces are marked experimental.
