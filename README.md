...
- **Native background delegation (experimental, off by default).** `cdx run --detach` can hand the run to Claude Code's own background agents (`--bg`, `claude agents`) instead of cdx's detached launcher, behind `CDX_EXPERIMENTAL_NATIVE_BG=1`. It is off because a live trial found the provider assigns its own session id under `--bg` and reports a pid belonging to a daemon helper, so cdx could not reliably identify the agent it had just started. `cdx run --detach` uses the in-house launcher, which is unchanged and is what every run takes today.
